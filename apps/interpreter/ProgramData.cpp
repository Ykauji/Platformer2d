...
#define CHECKARRAYRANGE(n,x) if ( (n)<0 || (n)>=(x) ) \
	...
	#define CHECKARRAYTYPE(n,t) if ( ((n)->m_iDataType & 0xFF) != (t) ) { RuntimeError("Incorrect array type"); return 1; }
#else
...
int ProgramData::LoadBytecodeHeader( const char* filename )
{
	...
	int dump = oBytecode.ReadInteger();

	int header_size = oBytecode.ReadInteger();
	if ( header_size >= 4 )
	{
		oBytecode.ReadString2( m_sCompanyName );
	}
...
int ProgramData::LoadBytecode( const char* filename )
{
	...
	for ( UINT i = 0; i < m_iNumVariablesArray; i++ )
	{
		...
		AllocArray( &(m_pVariablesArray[i]), type, typeIndex );
		...
		BlockResizeArray( &(m_pVariablesArray[i]), numDimensions, pLengths, type );
	...
	for ( UINT i = 0; i < pStruct->m_iNumVars; i++ )
	{
		switch( pStruct->m_pVarTypes[ i ].varType )
		{
			...
			case 5: // array
			{
				...
				AllocArray( pArray, pStruct->m_pVarTypes[ i ].varSubType, pStruct->m_pVarTypes[ i ].varTypeIndex );
				int dataType = pStruct->m_pVarTypes[ i ].varSubType;
				if ( dataType == 4 ) dataType |= (pStruct->m_pVarTypes[ i ].varTypeIndex << 8);
				BlockResizeArray( pArray, pStruct->m_pVarTypes[ i ].numDimensions, pStruct->m_pVarTypes[ i ].varLength, dataType );
...
void ProgramData::AllocArray( stArray *pArray, UINT type, UINT subType )
{
	pArray->m_iDataType = (subType << 8) | (type & 0xff);
	pArray->m_iLength = 0;
	pArray->m_iTotalLength = 0;
	pArray->m_pI = 0;
}

void ProgramData::SizeArray( stArray *pArray, UINT length, int finaltype )
{
	if ( length == 0 ) 
	{
		FreeArray( pArray );
	...
	UINT type = pArray->m_iDataType & 0xff;
	UINT subType = pArray->m_iDataType >> 8;
		...
		switch( type )
		{
			...
			case AGK_DATA_TYPE_ARRAY: // array
			{
				...
				for ( UINT i = 0; i < length; i++ )
				{
					...
					if ( finaltype > 0 ) pArray->m_pA[ i ]->m_iDataType = finaltype;
				}
				break;
			}
			default:
...
void ProgramData::CopyArray( stArray *pFromArray, stArray *pToArray )
{
	...
	UINT type = pFromArray->m_iDataType & 0xff;
...
void ProgramData::BlockResizeArray( stArray *pArray, UINT numDimensions, UINT dataType )
{
	...
	{
		// array of datatype
		if ( pArray->m_iDataType != dataType )
		{
			FreeArray( pArray );
			pArray->m_iDataType = dataType;
		}
	}
	else
	{
		// array of arrays
		if ( pArray->m_iDataType != AGK_DATA_TYPE_ARRAY )
		{
			FreeArray( pArray );
			pArray->m_iDataType = AGK_DATA_TYPE_ARRAY;
		}
	}
	...
	if ( numDimensions > 1 )
	{
		...
		{
			BlockResizeArray( pArray->m_pA[ i ], numDimensions-1, dataType );
		}
	}
}

void ProgramData::BlockResizeArray( stArray *pArray, UINT numDimensions, int *pLengths, UINT dataType )
{
	...
	if ( numDimensions > 1 )
	{
		...
		for( int i = 0; i < length; i++ )
		{
			BlockResizeArray( pArray->m_pA[ i ], numDimensions-1, pLengths+1, dataType );
		}
	}
}
...
int ProgramData::ParseDebugArray( AGKFunction *pFunction, int stackPtr, int strStackPtr, stArray *pArray, const char *szDimensions, uString &sValue, int &consumed )
{
	...
	if ( sRemaining.GetLength() == 0 )
	{
		int type = pArray->m_iDataType & 0xff;
		switch( type )
		{
			case AGK_DATA_TYPE_ARRAY:
			{
