case AGKI_NOP: // do nothing
	...
	case AGKI_ARRAY_BLOCK_RESIZE:
	{
		...
		BlockResizeArray( pArray, numDimensions, type );
	...
	case AGKI_ARRAY_REMOVE_TAIL:
	{
		...
		switch( pArray->m_iDataType & 0xff )
		{
	...
	case AGKI_ARRAY_SORT:
	{
		...
		switch( pArray->m_iDataType & 0xFF )
		{
			...
			case AGK_DATA_TYPE_STRING:
			{
				qsort( pArray->m_pS, pArray->m_iLength, sizeof(uString*), &ProgramData::CompareString );
				break;
			}
			case AGK_DATA_TYPE_TYPE:
			{
				stTypeDec *pTypeStruct = &(m_pTypeStructs[ pArray->m_iDataType >> 8 ]);
				...
				switch( varType )
				{
					...
					case AGK_DATA_TYPE_STRING:
					{
						...
						qsort( pValues2, pArray->m_iLength, sizeof(sortTypeString), &ProgramData::CompareTypeString );
						for ( UINT i = 0; i < pArray->m_iLength; i++ )
						{
